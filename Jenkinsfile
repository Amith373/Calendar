...
stage('Deploy') {
    steps {
        sshagent(['ec2-user']) {
            sh 'scp -o StrictHostKeyChecking=no sample.war ec2-user@54.226.130.212:/opt/tomcat/webapps/'
sh 'scp -o StrictHostKeyChecking=no sample.warubuntu@98.84.113.241:/opt/tomcat/tomcat10/webapps'
                    '''
                }
            }
        }  
